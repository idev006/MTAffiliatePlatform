...
async def main() -> None:
    ...
    parser.add_argument(
        "--evidence-set-id",
        default=None,
        help="Non-secret stable id grouping captures for one promotion review; autogenerated when omitted.",
    )
    parser.add_argument(
    ...
    evidence_set_id = args.evidence_set_id or f"p1-search-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}"
    capture_session_id = f"browser-session-{uuid.uuid4().hex[:12]}"
    out_dir = (
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            for result in results:
                ...
                if manifest_file:
                    manifests.append(json.loads(Path(str(manifest_file)).read_text(encoding="utf-8")))
            readiness = evaluate_program1_search_evidence_set(manifests)
            summary_payload = {
                "evidence_set_id": evidence_set_id,
                "capture_session_id": capture_session_id,
                "capture_results": results,
                "structural_promotion_readiness": readiness,
            }
